...
export async function POST(req: NextRequest) {
  try {
    const { term, brands, families, startDate, endDate, mode, adType } = await req.json();
    ...
    const buildQ = (s: string, e: string) => {
      let q = supabase.from(table)
        .select(`${dateField}, ad_type, final_match_type, campaign_name, impressions, clicks, spend, total_sales, total_orders`)
    ...
    type CampMap = Map<string,{ agg:Agg; adType:string }>;
    type MTMap   = Map<string,{ agg:Agg; adTypes:Set<string>; camps:CampMap }>;

    const buildMap = (rows: Record<string,unknown>[]): MTMap => {
      const mm: MTMap = new Map();
      for (const r of rows) {
        ...
        if (!mm.has(mtk)) mm.set(mtk,{agg:mk(),adTypes:new Set(),camps:new Map()});
        const me=mm.get(mtk)!; add(me.agg,r); me.adTypes.add(ak);
        const campKey=`${ak}||${ck}`;
        if (!me.camps.has(campKey)) me.camps.set(campKey,{agg:mk(),adType:ak});
        add(me.camps.get(campKey)!.agg,r);
      }
      return mm;
    };
    ...
    const matchTypes = Array.from(currMap.entries())
      .filter(([,me])=>me.agg.impressions>0)
      .map(([matchType,me]) => {
        const pme=prevMap.get(matchType);
        const campaigns = Array.from(me.camps.entries())
          .map(([campKey,ce]) => {
            const [adTypeName,campaignName] = campKey.split('||');
            return {
              campaignName, adType:adTypeName,
              ...withDeltas(ce.agg, pme?.camps.get(campKey)?.agg??null, me.agg.spend, me.agg.sales),
            };
          })
          .sort((a,b)=>b.spend-a.spend);
        return {
          matchType, adTypes:Array.from(me.adTypes),
          ...withDeltas(me.agg, pme?.agg??null, totalCurr.spend, totalCurr.sales),
          campaigns,
        };
      })
      .sort((a,b)=>b.spend-a.spend);
    ...
    return NextResponse.json({ data:matchTypes, total });
  } catch(err) {
    console.error('sta matchtype route error:', err);
